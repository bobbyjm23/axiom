# Sovereign Warden — Business Plan

**Version:** 2.1  
**Date:** July 2026  
**Status:** Active plan (founder path locked)

---

## For mentors and advisors

**Start here → read in this order:**

| # | Document | Time | What you'll learn |
|---|----------|------|-------------------|
| 1 | **[investor/mentor-brief.md](investor/mentor-brief.md)** | 5 min | What we're building, stage, and what feedback we need |
| 2 | **[plan.md](plan.md)** | 5 min | Full executive summary |
| 3 | **[investor/investor-faq.md](investor/investor-faq.md)** | 5 min | Hard investor questions answered |

**Then pick your lane:**

| If you care about… | Read |
|--------------------|------|
| Market and customers | [strategy/market-and-icp.md](strategy/market-and-icp.md) |
| Sales and 90-day plan | [strategy/gtm-pipeline.md](strategy/gtm-pipeline.md) |
| Competition | [strategy/competitive-positioning.md](strategy/competitive-positioning.md) · [Cetus/Songlines deep-dive](strategy/cetus-songlines-competitor-analysis.md) |
| Team and hiring | [operations/team-and-hiring.md](operations/team-and-hiring.md) |
| Product and delivery | [operations/product-and-delivery.md](operations/product-and-delivery.md) |
| Legal and entity | [operations/legal-and-compliance.md](operations/legal-and-compliance.md) |
| Funding and raise | [finance/funding.md](finance/funding.md) |
| Unit economics | [finance/unit-economics.md](finance/unit-economics.md) |
| **Hosted vs on-prem COGS** | [finance/hosted-vs-onprem-cogs.md](finance/hosted-vs-onprem-cogs.md) |
| Risks | [risk/risk-register.md](risk/risk-register.md) |

---

## For investors

| Document | Purpose |
|----------|---------|
| [plan.md](plan.md) | Executive plan |
| [investor/investor-faq.md](investor/investor-faq.md) | FAQ and traction |
| [investor/data-room-index.md](investor/data-room-index.md) | Due diligence checklist |
| [pitch-deck/index.html](pitch-deck/index.html) | Pitch deck (HTML) |
| [viability-review.md](viability-review.md) | Go/no-go score |

---

## Document library

### Strategy

| Document | Topic |
|----------|-------|
| [strategy/market-and-icp.md](strategy/market-and-icp.md) | ICP, TAM/SAM/SOM, buyer personas |
| [strategy/gtm-pipeline.md](strategy/gtm-pipeline.md) | 90-day plan, demo script, collateral |
| [strategy/competitive-positioning.md](strategy/competitive-positioning.md) | vs Copilot, Cetus, Macquarie |
| [strategy/cetus-songlines-competitor-analysis.md](strategy/cetus-songlines-competitor-analysis.md) | **Cetus / Songlines deep-dive** — features, pricing, GTM, competitive response roadmap |
| [strategy/target-accounts.md](strategy/target-accounts.md) | Priority A accounts and pipeline |
| [mid-market-track-a-strategy.md](mid-market-track-a-strategy.md) | Track A GTM (long-form) |
| [competitor-matrix.md](competitor-matrix.md) | Full feature matrix |

### Operations

| Document | Topic |
|----------|-------|
| [operations/team-and-hiring.md](operations/team-and-hiring.md) | Team, hiring, capacity |
| [operations/product-and-delivery.md](operations/product-and-delivery.md) | Product moat, runbook |
| [operations/legal-and-compliance.md](operations/legal-and-compliance.md) | Entity, insurance, Adneo conflict |

### Finance

| Document | Topic |
|----------|-------|
| [finance/funding.md](finance/funding.md) | Angel bridge, SAFE, use of funds |
| [finance/unit-economics.md](finance/unit-economics.md) | Pricing, margins, LTV:CAC |
| [finance/hosted-vs-onprem-cogs.md](finance/hosted-vs-onprem-cogs.md) | **AWS vs on-prem economics; post-wedge focus** |
| [finance/revenue-forecast.md](finance/revenue-forecast.md) | **Option A+B revenue forecast and charts** |
| [finance/valuation-and-exit.md](finance/valuation-and-exit.md) | **Y5 valuation and exit scenarios** |
| [financial-model-assumptions.md](financial-model-assumptions.md) | 3-year model inputs |
| [seed-funding-implications.md](seed-funding-implications.md) | Pre-seed and seed path |

### Risk

| Document | Topic |
|----------|-------|
| [risk/risk-register.md](risk/risk-register.md) | Risks, scenarios, kill criteria |

### Technical

| Document | Topic |
|----------|-------|
| [../architecture.md](../architecture.md) | Platform architecture |

### Reference

| Document | Notes |
|----------|-------|
| [sovereign-warden-business-plan.md](sovereign-warden-business-plan.md) | Comprehensive plan (pre-consolidation) |
| [archive/replan-july-2026/](archive/replan-july-2026/) | July 2026 workshop (historical audit trail) |

---

## Current status

| Area | Status | Next action |
|------|--------|-------------|
| Product | 🟡 ~3–4 weeks to production-ready | Demo env + runbook |
| Logo #1 | 🟡 Adneo (Lead) | Vendor clearance + pilot SOW |
| Entity | 🟡 AU Pty Ltd planned | Register + IP deed |
| Funding | 🟡 Angel bridge post-Adneo | 3 prospects, not contacted |
| Law GTM | ⬜ Starts week 7 | Priority A outbound |
| Y1 revenue target | **~$200k booked** | Adneo production + 2–3 law pilots |

---

*July 2026: Domain docs promoted from [replan workshop](archive/replan-july-2026/README.md). Update [plan.md](plan.md) when strategy changes.*
