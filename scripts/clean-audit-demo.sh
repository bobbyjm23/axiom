#!/usr/bin/env bash
# Remove Harbour Capital Wealth Management audit demo data
set -euo pipefail
...
if docker compose --env-file .env ps --status running anythingllm 2>/dev/null | grep -q anythingllm; then
  echo "==> Removing audit demo via AnythingLLM container..."
  ...
  docker compose --env-file .env exec -T anythingllm \
    node /app/extensions/warden-audit/scripts/demo-data.js clean
elif docker compose --env-file .env ps --status running postgres 2>/dev/null | grep -q postgres; then
  echo "==> Removing audit demo via local Node (postgres reachable on localhost)..."
  LOCAL_URL="${DATABASE_URL//@postgres:/@localhost:}"
  DATABASE_URL="${LOCAL_URL}" node extensions/warden-audit/scripts/demo-data.js clean
else
  echo "Start postgres first, or set DATABASE_URL and run:"
  echo "  node extensions/warden-audit/scripts/demo-data.js clean"
  exit 1
fi
